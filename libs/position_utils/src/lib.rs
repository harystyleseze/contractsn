...
use soroban_sdk::{contracterror, Address, BytesN, Env};
...
pub enum PositionError {
    ...
    LeverageExceedsMaximum = 2,
    OpenInterestExceeded   = 3,
}
...
pub fn validate_position(
    ...
    if min_collateral_factor > 0 {
        ...
        if collateral_usd < required_min {
            soroban_sdk::panic_with_error!(env, PositionError::CollateralBelowMinimum);
        }
    }
    ...
    if max_leverage > 0 && collateral_usd > 0 {
        ...
        if effective_leverage > max_leverage {
            soroban_sdk::panic_with_error!(env, PositionError::LeverageExceedsMaximum);
        }
    }

    // 3. OPEN INTEREST check
    if validate_open_interest(env, data_store, market, position.is_long).is_err() {
        soroban_sdk::panic_with_error!(env, PositionError::OpenInterestExceeded);
    }
}
